...
public class ServicioLoginImplTest {
    ...
    @Test
    public void debePoderConsultarUnUsuarioPorMail(){

        Usuario roberto = new Usuario();
        roberto.setEmail("roberto@noReply.com.ar");

        Mockito.when(repositorioUsuario.buscarUsuario(anyString())).thenReturn(roberto);

        Usuario resultado = servicioLogin.consultarUsuario("roberto@noReply.com.ar");
    ...
    public void debePermitirQueSeRegistreUnNuevoUsuario() throws UsuarioExistente {
        ...
        Mockito.when(repositorioUsuario.buscarUsuario(anyString())).thenReturn(null);
